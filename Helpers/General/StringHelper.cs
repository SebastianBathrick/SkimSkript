...
internal static class StringHelper
{
    ...
    private static string GetSpaceBeforeDigit(string input, int index) => index <= 0 || !char.IsDigit(input[index]) || !char.IsLetter(input[index - 1]) ? string.Empty : PASCAL_CASE_SPACING;
    ...
    private static string GetSpaceBeforeUpper(string input, int index)
    {
        ...
        return !char.IsLower(previous) && !char.IsDigit(previous) ? string.Empty : PASCAL_CASE_SPACING;
    }
    #endregion
}
