...
public static class OperationInterpreter
{
    public static Node PerformMathOperation(Node left, Node right, MathOperator mathOp)
    {
        ...
        switch (leftValueNode.GetType())
        {
            case Type t when t == typeof(IntValueNode):
                var intLeft = leftValueNode.ToInt();
                var intRight = rightValueNode.ToInt();
            ...
            case Type t when t == typeof(FloatValueNode):
                var floatLeft = leftValueNode.ToFloat();
                var floatRight = rightValueNode.ToFloat();
            ...
            case Type t when t == typeof(BoolValueNode):
                var leftBool = leftValueNode.ToBool();
                var rightBool = rightValueNode.ToBool();
            ...
            case Type t when t == typeof(StringValueNode):
                var leftString = leftValueNode.ToString();
                var rightString = rightValueNode.ToString();
    ...
    public static Node PerformLogicOperation(Node left, Node right, LogicOperator logicOp) => left is not BoolValueNode leftBoolNode || right is not BoolValueNode rightBoolNode
            ? throw new InvalidOperationException(
