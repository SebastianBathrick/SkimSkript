...
internal class Interpreter : MainComponent<AbstractSyntaxTreeRoot, int>
{
    ...
    #region Data Members
    private readonly ScopeManager _scope = new();
    ...
    private readonly ValueNode? _exitCodeNode;
    #endregion
    ...
    private Node? RunFunctionCall(FunctionCallNode functionCallNode)
    {
        ...
        return callableNode is FunctionNode userFunction
            ? RunUserFunction(userFunction, evaluatedArgs)
            : RunBuiltInFunction((BuiltInFunctionNode)callableNode, evaluatedArgs);
    }
    ...
    private BlockExitData RunIfStructure(IfNode ifNode)
    {
        ...
        if (CoercionInterpreter.CoerceCondition(evaluatedCondition))
            ...
            return ifNode.ChainedStructure is ElseIfNode elseIfNode
                ? RunElseIfStructure(elseIfNode)
                : RunElseStructure((ElseNode)ifNode.ChainedStructure);
    ...
    private BlockExitData RunElseIfStructure(ElseIfNode elseIfNode)
    {
        ...
        if (CoercionInterpreter.CoerceCondition(evaluatedCondition))
            ...
            return elseIfNode.ChainedStructure is ElseIfNode elseIfNodeChained
                ? RunElseIfStructure(elseIfNodeChained)
                : RunElseStructure((ElseNode)elseIfNode.ChainedStructure);
    ...
    private BlockExitData RunTryCatch(TryCatchNode tryCatchNode)
    {
        ...
        catch (Exception e)
        {
            if (tryCatchNode.CatchMessage != null)
            {
                ...
                var pointer = (StringValueNode)_scope.GetVariablePointer(msgVar.IdentifierNode.ToString());
    ...
    private Node EvaluateExpression(Node node)
    {
        ...
        return node is MathExpressionNode mathExpr ? EvaluateMathExpression(mathExpr) : EvaluateFactor(node);
    }
    ...
    private Dictionary<string, CallableNode> GetFunctionMap(Node[]? userDefinedFunctions)
    {
        ...
        foreach (var function in BuiltInFunctionNode.GetFunctionInstances())
