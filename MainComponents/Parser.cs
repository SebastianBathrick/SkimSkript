...
internal class Parser : MainComponent<TokenContainer, AbstractSyntaxTreeRoot>
{
    ...
    {
        Node newNode;
        var lexemeStartIndex = Tokens.GetLexemeStartIndex();
        ...
        switch (tokenType)
        {
            ...
            case TokenType.Try:
                ...
                break;
            default:
                newNode = IsDataTypeToken(tokenType) ? GetVariableDeclaration() : throw Tokens.GetTokenExceptionError(TokenType.StatementStartToken);
                break;
        }
    ...
    private Node GetVariableDeclaration()
    {
        ...
        return IsCollectionDataType(Tokens.PeekType()) ? GetCollectionDeclaration() : GetValueTypeVariableDeclaration();
    }

    private Node GetCollectionDeclaration() => throw new NotImplementedException("Collection declarations are not yet implemented.");
    ...
    private Node ParseComparisonExpression()
    {
        ...
        {
            Tokens.Remove();
            var rightNode = ParseArithmeticExpression(); // Comparison operators act on arithmetic expressions
    ...
    private Node ParseArithmeticExpression()
    {
        ...
        {
            Tokens.Remove();
            var rightNode = ParseTerm(); // Low-precedence arithmetic (add/subtract) acts on terms
    ...
    private Node ParseExponentTerm()
    {
        ...
        {
            Tokens.Remove();
            var exponent = ParseExponentTerm(); // Recursive call for right associativity
    ...
    private Node ParseFactor()
    {
        ...
        var lexeme = Tokens.RemoveAndGetLexeme();
    ...
    /// </summary>
    private Node GetIdentifierFactor() => Tokens.TryPeekAheadType(out var aheadType) && aheadType == TokenType.ParenthesisOpen
            ? GetFunctionCall()
            : new IdentifierNode(Tokens.RemoveAndGetLexeme());

    /// <summary>
    /// Returns either a function call or a variable assignment statement where the first token is an identifierNode.
    /// </summary>
    private Node GetIdentifierStartStatement() => Tokens.TryPeekAheadType(out var tokenType) && tokenType == TokenType.AssignmentOperator ? GetAssignment() : GetFunctionCall();
    #endregion
